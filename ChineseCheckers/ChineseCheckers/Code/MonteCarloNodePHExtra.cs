...
namespace ChineseCheckers
{
    ...
    class MonteCarloNodePHExtra
    {
        ...
        public Action act; // the action that spawned this node
        internal int playerIndex;
        internal MonteCarloNodePHExtra parent;
        internal Board board; // starea jocului la momentul reprezentat de acest nod
        internal LinkedList<MonteCarloNodePHExtra> children;
        ...
        public MonteCarloNodePHExtra(Board _board, MonteCarloNodePHExtra _parent, int parentPlayerIndex, Action action)
        {
            act = action;
            board = _board;
            parent = _parent;
            ...
            children = new LinkedList<MonteCarloNodePHExtra>();
            ...
            if (!board.hasWon(playerIndex))
                ...
                if (playerIndex == AIPlayerIndex) {
                    ...
                    victories = 1000;
                    totalGames = 1000;
                    MonteCarloNodePHExtra node = this; // backpropagate
        ...
        public MonteCarloNodePHExtra(Board _board, MonteCarloNodePHExtra _parent, int parentPlayerIndex,
            Action action, bool root)
        {
            ...
            if (!board.hasWon(playerIndex))
                ...
                if (playerIndex == AIPlayerIndex) {
                    victories = 1000;
                    totalGames = 1000;
                    MonteCarloNodePHExtra node = this;
                    while (node.parent != null) {
                        node.parent.totalGames += 1000;
                        node.parent.victories += 1000;
                        node = node.parent;
        ...
        public MonteCarloNodePHExtra select()
        {
            MonteCarloNodePHExtra node = this;
            ...
            while(node != null && node.unexploredActions.Count == 0) {
                ...
                double maxScore = -1;
                MonteCarloNodePHExtra mostPromising = null;
                foreach(MonteCarloNodePHExtra child in node.children) {
        ...
        public MonteCarloNodePHExtra expand()
        {
            ...
            MonteCarloNodePHExtra child = new MonteCarloNodePHExtra(newBoard, this, playerIndex, a);
            int actionScore;
            Game1.aiHistory[child.playerIndex].TryGetValue(a, out actionScore);
            //actionScore *= 4; // increase the impact of history
            if (actionScore < 0)
                child.totalGames -= actionScore;
            else {
                child.victories += actionScore;
                child.totalGames += actionScore;
            }
        ...
        // promising child
        public MonteCarloNodePHExtra getBestResult()
        {
            double maxScore = -1;
            MonteCarloNodePHExtra mostPromising = null;
            foreach(MonteCarloNodePHExtra child in children) {
                double score = child.victories;
                score /= child.totalGames;
